 ...
 *     be present.
 *
 * Claims are read only to *reject* before the signature is checked (a cheap
 * refusal that also avoids fetching the JWKS for junk); nothing is authorized
 * on until the signature holds. Which target a verified token may write —
 * a PR preview or the published store — is decided from `event_name`/`ref`
 * by `preview.ts` and gated by the publisher's scope in `auth-db.ts`.
 */
...
/**
 * Ceiling on the token itself. A real Actions token is ~1 KB; refusing an
 * absurd one costs nothing and keeps base64/JSON work bounded on junk input.
 */
const MAX_TOKEN_BYTES = 16 * 1024;

/**
 * Bound on the discovery/JWKS fetches. Without one a hung endpoint would hold
 * an upload request open until the client gave up.
 */
...
export interface GithubOidcClaims {
  ...
  ref?: string;
  /**
   * Event that triggered the run (`pull_request`, `push`, …). Together with
   * `ref` this decides whether the run owns a PR preview; see
   * `previewRefFromClaims` in `preview.ts`.
   */
  event_name?: string;
 ...
 *
 * Compared in constant time. The audience is not a secret, but it is the check
 * that stops a token minted for another service being replayed here, and a
 * length-then-bytes comparison is cheap enough that there is no reason to give
 * a timing signal on it.
 */
function audienceMatches(aud: unknown, expected: string): boolean {
  const expectedBuf = Buffer.from(expected);
  const oneMatches = (a: unknown): boolean => {
    if (typeof a !== "string") return false;
    const buf = Buffer.from(a);
    return buf.length === expectedBuf.length && timingSafeEqual(buf, expectedBuf);
  };
  if (typeof aud === "string") return oneMatches(aud);
  if (Array.isArray(aud)) return aud.some(oneMatches);
  return false;
}
...
export async function verifyGithubOidcToken(
  ...
  if (token.length > MAX_TOKEN_BYTES) return { ok: false, error: "token too large" };
  ...
  return {
    ...
    claims: {
      ...
      event_name: requireString(claims, "event_name") ?? undefined,
