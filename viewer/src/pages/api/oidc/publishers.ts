...
//        body: { project, repository, workflowRef, environment?, scope? }
...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  // What this trust may publish. Absent means previews: the low-risk grant,
  // and the one a project enrolling for PR doc previews actually wants.
  const scope = body.scope === undefined || body.scope === null ? "preview" : body.scope;
  if (!isPublisherScope(scope)) {
    return respond(
      { ok: false, error: `scope must be one of ${PUBLISHER_SCOPES.join(", ")}` },
      400
    );
  }
